...
#会在飞行途中转向，加入了位置检测，没有偏航角度检测，适合快速飞行
...
path = [
    ...
    [5, 0, -10.0, math.radians(0)],       # 向北飞 5 米
    [5, 0, -10.0, math.radians(90)],       #转向
    [5, 5, -10.0, math.radians(90)],  # 转向正东飞
    [5, 5, -10.0, math.radians(180)],  # 转向正东
    [0, 5, -10.0, math.radians(180)], # 转向正南飞
    [0, 5, -10.0, math.radians(0)],
...
try:
    for point in path:
        ...
        print(f"正在前往航点: x={target_x}, y={target_y}, z={target_z}, yaw={target_yaw}")

        while True:
            # 1. 必须以高频持续发送目标指令（维持 Offboard 模式）
            send_position_with_yaw(target_x, target_y, target_z, target_yaw)
            
            # 2. 获取当前位置
            curr_pos = get_current_location(connection)
            
            # 3. 检查是否到达（增加超时或打印距离方便调试）
            if is_at_target(curr_pos, (target_x, target_y, target_z)):
                print(f"已到达航点！")
                break
